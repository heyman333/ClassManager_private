(function($) {
		...
		return {
			...
			build : function(obj, opts) {
				...
				if (!this.options.optional
						|| this._totalRows() > this.options.limit) {
					this._createNavigation();
					this._setPage();
				}

				if (this.options.onCreate)
					this.options.onCreate(obj);
						...
						.click(
								function(e) {
									...
									// Page button
										...
										paginator
												._setPage($(this).data('page') * 1);
									}
									// 'Previous' or 'Next' button
									else if ($(this).data('previous') !== undefined
											|| $(this).data('next') !== undefined) {
										var page = paginator._currentPage()
												+ direction;
										if (page >= 0
												&& page <= paginator
														._totalPages()) {
									...
									// 'Last' button
										...
										paginator._setPage(paginator
												._totalPages());
									}

									// Handle callback
									if (paginator.options.onSelect)
										paginator.options.onSelect(
			...
			_setPage : function(index) {
				...
				this._rows().hide().slice(index * this.options.limit,
						(index + 1) * this.options.limit).show();

				// Set the current button as active
				this.nav.find('a').removeAttr('data-selected').siblings(
						'a[data-page=' + index + ']').attr('data-selected',
						true);
			},
	...
	$.fn.paginate = function(options) {
		...
		default:
			return Paginator().build(this,
					$.extend({}, $.fn.paginate.defaults, options));
		}
	};
